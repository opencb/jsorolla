...
export default class VariantAction {

    setGenomicLocation = value => {
    ...
    selectCaseVariantBrowser = caseName =>{
        cy.get("table").within(() => {
    ...
    setClinicalAnnotation = (filter, value) => {
    ...
    setConsequenceType = (filter, value) => {
        const filters = {
    ...
    setCohortAlternateStats = (cohort, filter, opt, value) => {
    ...
    selectPopulationFrequency = population => {
        // Show Collapse
        cy.get(`i[data-cy='pop-freq-toggle-${population}']`).click();
    };

    setPopulationFrequency = (population, filter, opt, value) => {
    ...
    setPopulationFrequencyInterpreter = (population, filter, opt, value) => {
    ...
    setHpoAccesions = value => {
    // HPO Accessions
        cy.get("hpo-accessions-filter .select2").first().click({force: true})
            .find(".select2-search__field").type(value + "{enter}", {delay: 200});
    };

    setProteingSubsScore = (filter, score, opt, value) =>{
        switch (filter) {
            case "sift":
            case "polyphen":
    ...
    showVariantBrowserTab = (component, tab) => {
