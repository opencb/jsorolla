...
//                    };
                    region : queriesList[i].toString().toLowerCase()
                    console.log(region)
                    var url = 'http://127.0.0.1:9090/region/'+region.chromosome+'/'+region.start+'/'+region.end+'/data';
...
//                    url = Utils.addQueryParamtersToUrl(queryParams, url);
                    ...
                    $.ajax({
                        ...
                        url:url,
                        success: function (data) {
                            _this._fetchSuccess(data, dataType);
                        }
                    });
                }
            }
    ...
    _fetchSuccess: function (data, dataType) {
        ...
        var chunks = [];
        var chunk = this.cache[dataType].putByRegion(data.id, JSON.parse(data.result));
        chunks.push(chunk);
//        for (var i = 0; i < data.length; i++) {
//            var queryResult = data[i];
//debugger
