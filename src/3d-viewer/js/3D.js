...
Viewer.Layer = function(numDisk, numLayer, config){
    ...
    this.width = (config.torusRadius-this.radius)*2*Math.PI/config.numDisk*config.diskWidth;
...
Viewer.Layer.loadShaders = function(urlVertexShader, urlFragmentShader){
    ...
    if(urlVertexShader !== undefined){
        $.ajax({
            async: false,
            url: urlVertexShader,
            success: function (data) { Viewer.Layer.vertexShader = $(data)[0].innerText; },
            dataType: 'html'
        });
    }
    if(urlFragmentShader !== undefined){
        $.ajax({
            async: false,
            url: urlFragmentShader,
            success: function (data) { Viewer.Layer.fragmentShader = $(data)[0].innerText; },
            dataType: 'html'
        });
    }
};
