...
export default class VariantUtils {
    ...
    static jsonToTabConvert(json, studiesPopFrequencies, samples, nucleotideGenotype, fieldList) {
        ...
        for (const v of json) {
            ...
            if (flatFieldList.includes("id")) {
                data["id"] = v.chromosome + ":" + v.start + " " + v.reference + "/" + v.alternate;
                // SNP ID
            ...
            alleleStats.forEach(study => {
                if (flatFieldList.includes(`cohorts.${study.id}`) || flatFieldList.includes("frequencies.cohort")) {
                    ...
                    data[`cohorts.${study.id}.alleleCount`] = ac.join(";");
                    data[`cohorts.${study.id}.altAlleleFreq`] = af.join(";");
                }
            });
            ...
            studiesPopFrequencies.forEach(study => {
                study.populations.forEach(pop => {
                    if (flatFieldList.includes("popfreq." + study.id) || flatFieldList.includes("frequencies.populationFrequencies")) {
                        ...
                        data[`popfreq.${study.id}_${pop.id}`] = UtilsNew.isNotEmpty(valuePopFreq) ? valuePopFreq : "-";
                    }
                });
            });

            if (flatFieldList.includes("clinicalInfo.clinvar")) {
                data["clinicalInfo.clinvar"] = clinvar;
            }

            if (flatFieldList.includes("clinicalInfo.cosmic")) {
                data["clinicalInfo.cosmic"] = cosmic;
            }

            if (flatFieldList.includes("interpretation.prediction")) {
                data["interpretation.prediction"] = prediction;
            }
            const rowValues = headerString.map(head => data[head]);
    ...
    static jsonToTabConvertOld(json, studiesPopFrequencies, samples, nucleotideGenotype, fieldList) {
        const rows = [];
        const dataString = [];
        const variantString = [];
        let populationMap = {};
        ...
        for (let i = 0; i < json.length; i++) {
            ...
            // gt samples
            row.push(...this.getGenotypeSamplesOld(v, samples, nucleotideGenotype));
    ...
    static getGenotypeSamplesOld(json, samples, nucleotideGenotype) {
        // Samples genotypes
