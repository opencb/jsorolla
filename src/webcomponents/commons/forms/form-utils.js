...
export default class FormUtils {
    ...
    static createObjectTest(_original, original, param, value) {
        const data = {...original};
        ...
        if (param.includes("[].")) {
            ...
            if (match?.groups?.field) {
                // 1.1.1 Check if we are updating an item just ADDED, example: 'phenotypes[].1'
                if (data[match?.groups?.arrayFieldName + "[]." + match?.groups?.index]) {
                    data[match?.groups?.arrayFieldName + "[]." + match?.groups?.index].after[match.groups.field] = value;
                } else {
                    // 1.1.2 We are updating an existing field, example: 'phenotypes[].0.name'
                    const originalArray = UtilsNew.getObjectValue(_original, match?.groups?.arrayFieldName, undefined);
                    data[param] = {
                    ...
                    if (data[param].before === data[param].after) {
                        delete data[param];
                    }
                }
            } else {
                ...
                if (value) {
                    ...
                    const keys = Object.keys(data).filter(key => key.startsWith(arrayFieldName + "[]."));
                    for (const key of keys) {
                        const split = key.split("[].")[1];
                        let keyIndex, newKey;
                        if (split.includes(".")) {
