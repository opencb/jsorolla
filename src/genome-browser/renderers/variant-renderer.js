...
class VariantRenderer extends Renderer {
    ...
    init(svgGroup, sample) {
        ...
        if (typeof this.sampleNames !== "undefined" && this.sampleNames !== null) {
            ...
            for (let i = 0; i < this.sampleNames.length; i++) {
                let sampleSvg = SVG.addChild(this.track.main, "text", {
                    "x": 0,
                    "y": y,
                    "stroke": "black",
                    "stroke-width": 1,
    ...
    _renderExtendedGenotypes(features, args) {
        ...
        for (let feature of features) {
            ...
            let ys = 5;         // y
            let yi = 10;         // y increment
            let yi2 = this.sampleTrackHeight;       // y increment

            let samplesCount = feature.studies[0].samplesData.length;
            for (let i in feature.studies[0].samplesData) {
                let svgPath = "M" + xs + "," + ys + " L" + xe + "," + ys + " L" + xe + "," + (ys + yi) + " L" + xs + "," + (ys + yi) + " z ";

                // Only one study is expected, and GT is always the first field in samplesData
                let genotype = feature.studies[0].samplesData[i]["0"];
                switch (genotype) {
                    case "0|0":
                    case "0/0":
                        d00 += svgPath;
