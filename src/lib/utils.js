...
var Utils = {
  ...
  myRound(value, decimals) {
    ...
    /** rounding * */
    if (Math.abs(value) >= 1) { value = value.toFixed(decimals); } else { value = value.toPrecision(decimals); }
    value = parseFloat(value);
    return value;
  },
  ...
  timeDiff(timeStart, timeEnd) {
    ...
    const hours = date_diff.getHours() - 1;
    let hoursMessage = `${hours} hour`;
    let minutesMessage = `${date_diff.getMinutes()} minute`;
    let secondsMessage = `${date_diff.getSeconds()} second`;
    if (hours !== 1) {
      hoursMessage += 's ';
    } else {
      hoursMessage += ' ';
