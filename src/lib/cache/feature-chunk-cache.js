...
FeatureChunkCache.prototype = {
    ...
    getAdjustedRegions: function (region, callback, keySuffix) {
        ...
        this.getChunks(keys, function(chunks){
            var regions = [];
            var chunkStart = parseInt(firstChunkId * temporalChunkSize);

            for (var i = 0; i < chunks.length - 1; i++) {
                if (!chunks[i]) {
                    if (chunks[i+1]) {  // missing chunks region ends here. build it.
                        var chunkEnd = parseInt(((firstChunkId + i) * temporalChunkSize) + temporalChunkSize - 1);
                        var r = new Region({chromosome: region.chromosome, start: chunkStart, end: chunkEnd});
                        regions.push(r);
                    }
                } else {
                    chunkStart = parseInt((firstChunkId + i + 1) * temporalChunkSize);  // prepare start for next chunk
                }
            }

            if (!chunks[chunks.length - 1]) {
                var chunkEnd = parseInt(((lastChunkId) * temporalChunkSize) + temporalChunkSize - 1);
                var r = new Region({chromosome: region.chromosome, start: chunkStart, end: chunkEnd});
                regions.push(r);
            }
            callback(regions);
    ...
    putByRegions: function (regionArray, valueArray, category, dataType, chunkSize) { // encoded
        ...
        return this.putChunks(category, chunkKeyArray, valueArray, false);
    },
