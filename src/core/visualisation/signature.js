...
export default class Signature {
    ...
    constructor(signature, mode, config) {
    ...
    render(targetDiv, config) {
        if (config) {
            this._config = {...this.getDefaultConfig(), ...config};
        }

        return this.#renderCounts(targetDiv);
    }

    #renderCounts(targetDiv) {
        ...
        $(`#${targetDiv}`).highcharts({
            title: {
                text: `${counts.reduce((c, s) => c + s.total, 0)} ${mode === "SBS" ? "Substitutions New" : "Rearrangements"}`,
            },
            chart: {
