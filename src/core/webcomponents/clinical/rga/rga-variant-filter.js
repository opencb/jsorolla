...
export default class RgaVariantFilter extends LitElement {
    ...
    _createSubSection(subsection) {
        ...
        switch (subsection.id) {
            case "id":
                ...
                break;
            case "region":
                content = html`<region-filter placeholder="${subsection.placeholder}" .value="${this.preparedQuery[subsection.id]}" .separator="${",;"}" @filterChange="${e => this.onFilterChange(subsection.id, e.detail.value)}"></region-filter>`;
                break;
            case "type":
...
customElements.define("rga-variant-filter", RgaVariantFilter);
