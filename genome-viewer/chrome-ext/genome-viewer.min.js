...
DasRegionDataAdapter.prototype.fill=function(a,b,c){var d=this;if(null==a||null==b||null==c)throw"Missing value in a not optional parameter: chromosome, start or end";var e=this.proxy+this.url+"?segment="+a+":"+b+","+c;console.log(e);if(this.isRegionAvalaible(a,b,c))return this.lockSuccessEventNotify=!1,this.datasets[this._getHashMapKey(a,b,c)];$.ajax({url:e,type:"GET",dataType:"xml",error:function(){alert("error");d.onError.notify("It is not allowed by Access-Control-Allow-Origin ")},success:function(e){console.log("DATA FROM DAS: ");
console.log(e);try{d.xml=(new XMLSerializer).serializeToString(e);var g=(new XMLSerializer).serializeToString(e),e=xml2json.parser(g),g=[];if(null!=e.dasgff.gff.segment.feature){for(var h=0;h<array.length;h++)e.dasgff.gff.segment.feature[h].chromosome=a;g.push(e.dasgff.gff.segment.feature)}else g.push([]);d.lockSuccessEventNotify?d.anticipateRegionRetrieved(g,a,b,c):d.getFinished(g,a,b,c)}catch(k){alert("There was a problem parsing the xml: "+k),console.log(e)}}})};
...
InfoWidget.prototype.draw=function(a){this.feature=a;null==a.getName()&&console.log("getName not defined");console.log(a.getName());this.panel=Ext.getCmp(this.title+" "+this.feature.getName());null==this.panel?(this.render(),this.panel.show(),this.getData()):this.panel.show()};
...
function CellBaseManager(a){this.host="http://ws.bioinfo.cipf.es/cellbase/rest";if(-1!=window.location.host.indexOf("bioinfo.cipf.es")||-1!=window.location.host.indexOf("genomemaps.org")||-1!=window.location.host.indexOf("cellbrowser.org"))this.host="http://ws.bioinfo.cipf.es/cellbase/rest";this.version="v1";this.species=a;CELLBASEHOST=this.host;this.subcategory=this.category=null;this.contentformat="jsonp";this.fileformat="";this.outputcompress=!1;this.dataType="script";this.resource=this.query=
"";this.async=!0;this.maxQuery=20;this.numberQueries=0;this.results=[];this.resultsCount=[];this.batching=!1;this.completed=new Event;this.successed=new Event;this.batchSuccessed=new Event;this.error=new Event;this.setVersion=function(a){this.version=a};this.setSpecies=function(a){this.species=a};this.getVersion=function(){return this.version};this.getSpecies=function(){return this.species};this.setAsync=function(a){this.async=a};this.getUrl=function(){return null!=this.query?this.host+"/"+this.version+
"/"+this.species+"/"+this.category+"/"+this.subcategory+"/"+this.query+"/"+this.resource:this.host+"/"+this.version+"/"+this.species+"/"+this.category+"/"+this.subcategory+"/"+this.resource};this.get=function(a,c,d,e,f){if(d instanceof Array)this.batching=!0,this.results=[],this.getMultiple(a,c,d,e);else{var d=new String(d),d=d.replace(/\s/g,""),g=d.split(",");this.maxQuery<g.length?(this.batching=!0,this.getMultiple(a,c,g,e,f)):(this.batching=!1,this.getSingle(a,c,d,e,f))}};this._joinToResults=function(a){this.resultsCount.push(!0);
this.results[a.id]=a.data;if(this.numberQueries==this.resultsCount.length){for(var a=[],c=0;c<this.results.length;c++)for(var d=0;d<this.results[c].length;d++)a.push(this.results[c][d]);this.successed.notify(a)}};this.getSingle=function(a,c,d,e,f,g){this.category=a;this.subcategory=c;this.query=d;this.resource=e;this._callServer(this.getUrl(),f,g)};this.getMultiple=function(a,c,d,e){var f=[];if(d.length>this.maxQuery)for(var g=0;g<d.length;g+=this.maxQuery)f.push(d.slice(g,g+this.maxQuery));else f.push(d);
this.numberQueries=f.length;var h=this;this.batchSuccessed.addEventListener(function(a,b){h._joinToResults(b)});for(g=0;g<f.length;g++)this.results.push([]),this.getSingle(a,c,f[g].toString(),e,g)};this._callServer=function(a,c,d){var e=this;$.ajax({url:a,type:"GET",async:this.async,data:{of:this.contentformat,outputcompress:this.outputcompress,callbackParam:"response"},dataType:this.dataType,jsonp:"callback",success:function(){"undefined"!=typeof response?(null!=d&&d(response),e.batching?e.batchSuccessed.notify({data:response,
id:c}):e.successed.notify(response)):e.error.notify()},complete:function(){e.completed.notify()},error:function(){e.error.notify()}});console.log(a)}}ExpressionGenomicAttributesWidget.prototype.draw=GenomicAttributesWidget.prototype.draw;ExpressionGenomicAttributesWidget.prototype.getMainPanel=GenomicAttributesWidget.prototype.getMainPanel;ExpressionGenomicAttributesWidget.prototype.render=GenomicAttributesWidget.prototype.render;ExpressionGenomicAttributesWidget.prototype.drawKaryotype=GenomicAttributesWidget.prototype.drawKaryotype;
ExpressionGenomicAttributesWidget.prototype.makeGrid=GenomicAttributesWidget.prototype.makeGrid;ExpressionGenomicAttributesWidget.prototype.getKaryotypePanelId=GenomicAttributesWidget.prototype.getKaryotypePanelId;ExpressionGenomicAttributesWidget.prototype.onAdditionalInformationClick=GenomicAttributesWidget.prototype.onAdditionalInformationClick;
...
a.clientY)},!0);a.addEventListener("mouseout",function(){f.tooltippanel.destroy()},!0)};FeatureTrack.prototype.getIdToPrint=function(a){return a.id};FeatureTrack.prototype._render=function(){this.init();this.isAvalaible&&null!=this.features&&this.drawFeatures(this.features)};FeatureTrack.prototype.moveTitle=function(){};
FeatureTrack.prototype.drawTitle=function(a){this.titleNodeGroup.setAttribute("moveX",0);0>a&&(a=0);a=this._convertGenomePositionToPixelPosition(a);null!=this.titleRectangle&&this.titleRectangle.parentNode.removeChild(this.titleRectangle);null!=this.titleText&&this.titleText.parentNode.removeChild(this.titleText);if(this.isAvalaible){var b=[["fill","#FFFFFF"],["stroke","#000000"],["opacity",0.7],["rx",4],["ry",4],["class","trackTitleBox"]];this.titleRectangle=SVG.drawRectangle(a,this.top,this.titleWidth,
this.height,this.titleNodeGroup,b);this.titleText=SVG.drawText(a+2,this.top+this.titleHeight-3,this.titleName,this.titleNodeGroup,[["font-size",this.titleFontSize],["class","trackTitleText"]])}else b=[["fill","#FFFFCC"],["stroke","#000000"],["opacity",0.7],["rx",0],["ry",0],["class","trackTitleBox"]],SVG.drawRectangle(a,this.top,this.width,this.height,this.titleNodeGroup,b),SVG.drawText(a+2,this.top+this.height-4,this.titleName+": "+this.isNotAvalaibleMessage,this.titleNodeGroup,[["font-size",this.titleFontSize],
["class","trackTitleText"]])};FeatureTrack.prototype._addFeatures=function(a){this.features=a.toJSON();this.featuresIndex={}};FeatureTrack.prototype.draw=function(a,b,c){this.top=c;null!=b&&(this._svg=b);null!=a.toJSON()&&this._addFeatures(a);this._render()};FeatureTrack.prototype.mouseclick=function(){};
...
TrackCanvas.prototype._moveTitle=function(){var a=$(".trackTitleBox"),b=$(".trackTitleText"),c=this.start;0>c&&(c=0);for(var c=c*this.pixelRatio%this.viewBoxModule,d=0;d<a.length;d++)a[d].setAttribute("x",c),b[d].setAttribute("x",c+2)};
...
a.blockStarts+"</span><br>"));if(a instanceof GeneFeatureFormatter||a instanceof TranscriptFeatureFormatter||a instanceof ExonFeatureFormatter||a instanceof SNPFeatureFormatter||a instanceof TfbsFeatureFormatter||a instanceof MutationFeatureFormatter||a instanceof CpgIslandFeatureFormatter||a instanceof StructuralVariationFeatureFormatter||a instanceof MiRNAFeatureFormatter)b='<span class="ssel">'+a.getName()+'</span><br>start: <span class="emph">'+a.start+'</span><br>end:  <span class="emph">'+a.end+
'</span><br>length: <span class="info">'+(a.end-a.start+1)+"</span><br>";a instanceof VCFFeatureFormatter&&(b='<span class="ssel">'+a.getName()+"</span><br>");return b};
...
labelSize:this.labelSize,title:"Gene/Transcript",titleWidth:70,titleFontSize:9,labelFontSize:8,showTranscripts:!0,allowDuplicates:!1,backgroundColor:"#FFFFFF",label:!0,pixelSpaceBetweenBlocks:200,showDetailGeneLabel:!0,showLabelsOnMiddleMarker:!0,showExonLabel:!0,onMouseOverShitExonTranscriptLabel:!0});this.addTrackByZoom(95,100,a,new GeneRegionCellBaseDataAdapter(this.species))};
...
GenomeWidget.prototype.draw=function(a,b,c,d,e){this.chromosome=a;this.start=b;this.end=c;var f=this;this.init();this.trackCanvas=new TrackCanvas(this.id+"_canvas",document.getElementById(this.targetId),{top:0,left:0,right:this.width,width:this.width,height:this.height,start:this.start,end:this.end,backgroundColor:"#FFCCFF",pixelRatio:this.pixelRatio,viewBoxModule:this.getviewBoxModule(),allowDragging:this.allowDragging,lastPosition:this.lastPosition});this.trackCanvas.init();this.trackCanvas.afterDrag.addEventListener(function(){});
this.trackCanvas.onMove.addEventListener(function(a,b){f.onMarkerChange.notify(b)});for(a=0;a<this.trackList.length;a++)this.trackList[a].viewBoxModule=this.getviewBoxModule(),this.trackList[a].pixelRatio=this.pixelRatio,this.trackList[a].targetID=document.getElementById(this.targetId),this.trackCanvas.addTrack(this.trackList[a],this.dataAdapterList[a]);f=this;this.trackCanvas.onRender.addEventListener(function(){f.onRender.notify()});this.trackCanvas.draw(this.chromosome,this.start,this.end,d,e)};
...
"beta"),null!=c.menuBar&&(this.menuBar=c.menuBar),null!=c.width&&(this.width=c.width),null!=c.height&&(this.height=c.height),null!=c.pluginsMenu&&(this.pluginsMenu=c.pluginsMenu),null!=c.chromosome&&(this.chromosome=parseInt(c.chromosome)),null!=c.position&&(this.position=parseInt(c.position)));this.onSpeciesChange=new Event;this.onZoomChange=new Event;this.onLastChrPosition=new Event;console.log(this.width+"x"+this.height);console.log(this.targetId);console.log(this.id);this.genomeWidget=null;console.log(this.zoom);
this.genomeWidgetProperties=new GenomeWidgetProperties(this.species,{width:this.width,windowSize:1E6,pixelRatio:5.0E-4,id:this.id,zoom:this.zoom,increment:this.increment});this.onSpeciesChange.addEventListener(function(a,b){d.species=b.species;d.speciesName=b.name;Ext.getCmp(d.id+"speciesMenuButton").setText(d.speciesName);Ext.example.msg("Species",d.speciesName+" selected.");Ext.getCmp(d.id+"chromosomeMenuButton").menu=d._getChromosomeMenu();d.genomeWidgetProperties=new GenomeWidgetProperties(d.species,
{width:d.width,windowSize:1E6,pixelRatio:5.0E-4,id:d.id,zoom:d.zoom,increment:d.increment});d.setLocation(b.chromosome,b.position);null!=d.targetId&&d.draw()});this.customTracksAddedCount=1;this.drawing=0}GenomeViewer.prototype.setMenuBar=function(a){this.menuBar=a};GenomeViewer.prototype.draw=function(){null!=this.targetId&&this._getPanel(this.width,this.height);this._render()};
...
GenomeViewer.prototype._getZoomSlider=function(){var a=this;null==this._zoomSlider&&(this._zoomSlider=Ext.create("Ext.slider.Single",{id:this.id+" zoomSlider",width:200,minValue:0,hideLabel:!1,maxValue:100,value:this.zoom,useTips:!0,increment:this.increment,tipText:function(a){return Ext.String.format("<b>{0}%</b>",a.value)}}),this._zoomSlider.on("changecomplete",function(b,c){a._handleNavigationBar("ZOOM",c)}));return this._zoomSlider};
...
GenomeViewer.prototype._getChromosomePanel=function(){var a=Ext.create("Ext.container.Container",{id:this._getChromosomeLabelID(),margin:5}),b=Ext.create("Ext.container.Container",{id:this._getChromosomeContainerID(),margin:10});return Ext.create("Ext.container.Container",{height:80,layout:{type:"table",columns:2},items:[a,b]})};
GenomeViewer.prototype._getWindowSizeArrow=function(){var a=this.width-15,b=a-10;return'<svg xmlns="http://www.w3.org/2000/svg" version="1.1"><path fill="black" opacity="0.5" d="M0 15 L10 9 L10 21  Z" /><path fill="black" opacity="0.5" d="M10 16 L10 14 L'+b+" 14 L"+b+' 16 Z" /><path fill="black" opacity="0.5" d="M'+a+" 15 L"+b+" 9 L"+b+' 21 Z" /><text id="'+this.id+'windowSizeLabel" x="'+(a/2-60)+'" y="10" fill="blue">nt</text></svg>'};
GenomeViewer.prototype._getWindowSizePanel=function(){return Ext.create("Ext.container.Container",{id:this.id+"windowSizePanel",height:22,html:this._getWindowSizeArrow()})};GenomeViewer.prototype._getTracksPanelID=function(){return this.id+"master"};GenomeViewer.prototype._getTracksPanel=function(){null==this._mainPanel&&(this._mainPanel=Ext.create("Ext.panel.Panel",{autoScroll:!0,flex:1,border:!1,html:'<div height=2000px; overflow-y="scroll"; id = "'+this._getTracksPanelID()+'"></div>'}));return this._mainPanel};
...
GenomeViewer.prototype.drawChromosome=function(a){this._setChromosomeLabel(a);DOM.removeChilds(this._getChromosomeContainerID());var b=this.width-100;this.chromosomeFeatureTrack=new ChromosomeFeatureTrack(this.id+"chr",document.getElementById(this._getChromosomeContainerID()),this.species,{top:5,bottom:20,left:10,right:b-100,width:b,height:50,label:!0,vertical:!1,rounded:4});var c=this,d=new RegionCellBaseDataAdapter(this.species,{resource:"cytoband"});d.successed.addEventListener(function(a,b){null!=
b&&0<b[0].length&&(c.lastPosition=b[0][b[0].length-1].end,c.position>c.lastPosition&&(c.position=c.lastPosition/2),c.onLastChrPosition.notify());c.chromosomeFeatureTrack.draw(d.dataset);c.chromosomeFeatureTrack.click.addEventListener(function(a,b){c.position=Math.ceil(b);c.refreshMasterGenomeViewer()})});d.fill(a,1,26E7,"cytoband");this._getPanel().setLoading("Retrieving data")};
GenomeViewer.prototype._setScaleLabels=function(){var a=Math.floor(100/this.genomeWidgetProperties.getPixelRatio())+" nt ",b="Viewing "+Math.ceil((this.width-15)/this.genomeWidgetProperties.getPixelRatio())+" nts ";Ext.getCmp(this.id+"scaleLabel").surface.items.items[0].setAttributes({text:a},!0);Ext.getDom(this.id+"windowSizeLabel").firstChild.nodeValue=b};
GenomeViewer.prototype.setZoom=function(a){this.genomeWidgetProperties.setZoom(a);this.zoom=a;this.position=this.genomeWidget.getMiddlePoint();this._getZoomSlider().setValue(a);this._setScaleLabels();this.refreshMasterGenomeViewer()};GenomeViewer.prototype.setLocation=function(a,b){this.chromosome=a;this.position=Math.ceil(b);this.drawChromosome(this.chromosome,1,26E6);this.refreshMasterGenomeViewer()};
GenomeViewer.prototype._setPositionField=function(a,b){NaN==b&&(b=1E6);0>b?Ext.getCmp(this.id+"tbCoordinate").setValue(a+":0"):Ext.getCmp(this.id+"tbCoordinate").setValue(a+":"+Math.ceil(b))};GenomeViewer.prototype._getWindowsSize=function(){return this.genomeWidgetProperties.getWindowSize(this.zoom)};GenomeViewer.prototype.refreshMasterGenomeViewer=function(){this.updateRegionMarked(this.chromosome,this.position);this._drawGenomeViewer()};
GenomeViewer.prototype._drawGenomeViewer=function(){var a=this;a.drawing+=1;setTimeout(function(){a.drawing-=1;0==a.drawing&&a._drawOnceGenomeViewer()},500)};
GenomeViewer.prototype._drawOnceGenomeViewer=function(){var a=this;this._getPanel().setLoading("Retrieving data");null!=this.genomeWidget&&this.genomeWidget.clear();var b=this.genomeWidgetProperties.getPixelRatio();this.genomeWidget=new GenomeWidget(this.id+"master",this._getTracksPanelID(),{pixelRatio:b,width:this.width-15,lastPosition:this.lastPosition,height:2E3});var c=this.zoom;if(0<this.genomeWidgetProperties.getTrackByZoom(c).length){for(var d=0;d<this.genomeWidgetProperties.getTrackByZoom(c).length;d++){var e=
this.genomeWidgetProperties.getTrackByZoom(c)[d];e.top=e.originalTop;e.height=e.originalHeight;e.clear();this.genomeWidgetProperties.getDataAdapterByZoom(c)[d].datasets={};this.genomeWidget.addTrack(e,this.genomeWidgetProperties.getDataAdapterByZoom(c)[d])}c=Math.ceil(this.position-this._getWindowsSize());d=Math.ceil(this.position+this._getWindowsSize());e=(this.width-15)/b/2;b=Math.ceil(this.position-e);e=Math.ceil(this.position+e);0>c&&(c=0);this.genomeWidget.onMarkerChange.addEventListener(function(b,
c){a.position=Math.floor(c.middle);a._setPositionField(a.chromosome,Math.floor(c.middle));a.updateRegionMarked(a.chromosome,c.middle)});this.genomeWidget.onRender.addEventListener(function(){a._getPanel().setLoading(!1)});this.genomeWidget.draw(this.chromosome,c,d,b,e)}else a._getPanel().setLoading("No tracks to display")};GenomeViewer.prototype.updateRegionMarked=function(a,b){var c=this.genomeWidgetProperties.windowSize/2,d=b-c;0>d&&(d=0);this.chromosomeFeatureTrack.select(d,b+c)};
GenomeViewer.prototype.loadDASTrack=function(a,b){if(void 0==this.genomeWidgetProperties.tracks[a]){var c=new DasRegionDataAdapter({url:b});this.genomeWidgetProperties.addCustomTrackByZoom(0,50,new FeatureTrack("vcf",null,this.species,{top:10,left:0,height:10,title:a,opacity:0.9,titleFontSize:9,forceColor:"purple",label:!0,avoidOverlapping:!0,pixelSpaceBetweenBlocks:100,allowDuplicates:!0,backgroundColor:"#FCFFFF",isAvalaible:!0}),c);this.genomeWidgetProperties.addCustomTrackByZoom(60,100,new FeatureTrack("vcf",
null,this.species,{top:10,left:0,height:10,title:a,titleFontSize:9,forceColor:"#000000",label:!0,avoidOverlapping:!0,pixelSpaceBetweenBlocks:100,allowDuplicates:!0,backgroundColor:"#FCFFFF",isAvalaible:!0}),c)}};
