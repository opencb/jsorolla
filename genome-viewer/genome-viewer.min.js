...
k)for(var m=k+=7;m<l;m++)if('"'==g[m]){var p=g.substr(k,m-k),l=m;if(-1!=p.indexOf("="))for(;-1!=p.indexOf("=");)var p=p.replace("=","-"),s=g.substr(k,m-k),g=g.replace(s,p)}l=this.nodeAttributesToJSON(g.substr(g.indexOf("["),g.indexOf("];")));g=g.substr(0,g.indexOf("[")).replace(/^\s+|\s+$/g,"");d[g]=l}}for(var n in d)null==this.dataset.getVertexByName(d[n].label)&&(void 0==d[n].label&&(d[n].label=""),this.dataset.addNode(d[n].label,d[n]));for(h in b)a=this.dataset.getVertexByName(d[b[h].source].label),
...
(function(){function a(a){return 10>a?"0"+a:a}function b(a){e.lastIndex=0;return e.test(a)?'"'+a.replace(e,function(a){var b=h[a];return"string"===typeof b?b:"\\u"+("0000"+a.charCodeAt(0).toString(16)).slice(-4)})+'"':'"'+a+'"'}function c(a,d){var e,h,n,t,o=f,r,q=d[a];q&&"object"===typeof q&&"function"===typeof q.toJSON&&(q=q.toJSON(a));"function"===typeof k&&(q=k.call(d,a,q));switch(typeof q){case "string":return b(q);case "number":return isFinite(q)?""+q:"null";case "boolean":case "null":return""+
q;case "object":if(!q)return"null";f+=g;r=[];if("[object Array]"===Object.prototype.toString.apply(q)){t=q.length;for(e=0;e<t;e+=1)r[e]=c(e,q)||"null";n=0===r.length?"[]":f?"[\n"+f+r.join(",\n"+f)+"\n"+o+"]":"["+r.join(",")+"]";f=o;return n}if(k&&"object"===typeof k){t=k.length;for(e=0;e<t;e+=1)h=k[e],"string"===typeof h&&(n=c(h,q))&&r.push(b(h)+(f?": ":":")+n)}else for(h in q)Object.hasOwnProperty.call(q,h)&&(n=c(h,q))&&r.push(b(h)+(f?": ":":")+n);n=0===r.length?"{}":f?"{\n"+f+r.join(",\n"+f)+"\n"+
...
l=Array(80),m,p,s,n,t,e=0;e<c;e++){for(var o=0;16>o;o++)l[o]=d[e][o];for(o=16;80>o;o++)l[o]=Math.ROTL(l[o-3]^l[o-8]^l[o-14]^l[o-16],1);m=a;p=f;s=g;n=h;t=k;for(o=0;80>o;o++){var r=Math.floor(o/20),r=Math.ROTL(m,5)+Math.f(r,p,s,n)+t+b[r]+l[o]&4294967295;t=n;n=s;s=Math.ROTL(p,30);p=m;m=r}a=a+m&4294967295;f=f+p&4294967295;g=g+s&4294967295;h=h+n&4294967295;k=k+t&4294967295}return a.toHexStr()+f.toHexStr()+g.toHexStr()+h.toHexStr()+k.toHexStr()};
...
this.width)];this.draw()},getModel:function(){return this._model},addNode:function(a,b,c,d,e,f,g,h,k,l){var m=[],p=[];m.push(c);p.push(d);var s=[];s.push(c);s.push(d);c=new PlotNode(a,b,m,p,[],{color:e,size:h,opacity:g,shape:k,nodeStrokeWidth:l});d=this._model.getNodes();this._model.getNodesId()[a]=d.length;d.push(c);this._coordenates[a]=s;this._color[a]=e;this._visible[a]=f;this._opacity[a]=g;this._size[a]=h;this._shape[a]=k;this._nodeStrokeWidth[a]=l;this._deletedNodes[a]=!1;this._id_title[a]=b;
...
k=g.getEdgesCount(),l=(new SIFFileDataAdapter).toSIF(g),m=new TabularFileDataAdapter;m.parse(l);a.gridStore.loadData(m.getLines());a.infoLabel.setText('<span class="ok">File loaded sucessfully</span>',!1);a.countLabel.setText('vertices:<span class="info">'+h+'</span> edges:<span class="info">'+k+"</span>",!1)}catch(p){a.infoLabel.setText('<span class="err">File not valid </span>'+p,!1)}})}}})};
